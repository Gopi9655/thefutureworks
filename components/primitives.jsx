    ...
    <span className={"orb " + (spin ? "spin-slow " : "") + className}
      style={{
        ...
        boxShadow: "inset -6px -7px 16px rgba(0,0,0,.45), inset 5px 5px 12px rgba(255,255,255,.25)" +
          (glow ? ", 0 0 36px -2px var(--red-glow)" : ""),
        ...style,
      }}>
...
function Reveal({ children, d, as = "div", className = "", style, ...rest }) {
  ...
  const El = as;
  return (
    <El ref={ref} data-d={d} className={"reveal " + (seen ? "in " : "") + className} style={style} {...rest}>
      {children}
    </El>
  );
}
...
/* ============================================================
   BUTTON / LINK helpers
   ============================================================ */
function Btn({ to, href, variant = "primary", size = "", icon, iconLeft, children, onClick, className = "", style }) {
  ...
  return <a href={href || (to ? "#" + to : "#")} className={cls} style={style} onClick={handle} target={href ? "_blank" : undefined} rel={href ? "noopener" : undefined}>{content}</a>;
}
...
function SkipLink() {
  return (
    <a href="#main-content" className="skip-link" style={{
      position: "fixed", top: -60, left: 16, zIndex: 999,
...
Object.assign(window, { Icon, Orb, Logo, Reveal, CountUp, Btn, SectionHead, go, SkipLink, useState, useEffect, useRef, useCallback });
